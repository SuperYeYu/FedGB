...
from fedgb.algorithms.graph_fgl.gcfl_plus.gcfl_plus_config import config
import numpy as np
from fedgb.algorithms.graph_fgl.gcfl_plus.models import CrossDomainGIN
from fedgb.algorithms.graph_fgl.gcfl_plus.utils import sync_cluster_weights_to_clients
...
class GCFLPlusServer(BaseServer):
    ...
    def __init__(self, args, global_data, data_dir, message_pool, device):
        ...
        self.task.load_custom_model(
            CrossDomainGIN(
                ...
                model_name=getattr(self.args, "model", ["gin"]),
                edge_dim=getattr(self.args, "edge_dim", None),
            ))
        ...
        self.EPS_1 = getattr(args, "gcfl_eps1", config['eps1'])
        self.EPS_2 = getattr(args, "gcfl_eps2", config['eps2'])
        self.seq_length = getattr(args, "gcfl_plus_seq_length", config['seq_length'])
        self.standardize = getattr(args, "gcfl_plus_standardize", config['standardize'])
        self.min_cut_round = getattr(args, "gcfl_min_cut_round", 20)
    ...
    def execute(self):
        ...
        for idc in self.cluster_indices:
            ...
            if mean_norm < self.EPS_1 and max_norm > self.EPS_2 and len(idc) > 2 and self.message_pool["round"] > self.min_cut_round \
    ...
    def sync_clients_after_aggregation(self, clients):
        sync_cluster_weights_to_clients(self.cluster_indices, self.cluster_weights, clients)
    ...
    def compute_pairwise_distances(self, seqs, standardize=False):
        ...
        if standardize:
            ...
            seqs = np.array(seqs)
        distances = np.zeros((len(seqs), len(seqs)), dtype=float)
        for i in range(len(seqs)):
            for j in range(i + 1, len(seqs)):
                distances[i, j] = distances[j, i] = dtw_distance(seqs[i], seqs[j])
        return distances
...
def dtw_distance(seq_a, seq_b):
    ...
    dp = np.full((len(seq_a) + 1, len(seq_b) + 1), np.inf, dtype=float)
    dp[0, 0] = 0.0
    for i in range(1, len(seq_a) + 1):
        for j in range(1, len(seq_b) + 1):
            cost = abs(seq_a[i - 1] - seq_b[j - 1])
            dp[i, j] = cost + min(dp[i - 1, j], dp[i, j - 1], dp[i - 1, j - 1])
    return float(dp[len(seq_a), len(seq_b)])
