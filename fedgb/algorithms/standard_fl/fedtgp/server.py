...
class FedTGPServer(BaseServer):
    ...
    def __init__(self, args, global_data, data_dir, message_pool, device, fedtgp_lambda=1, num_glb_epochs=10, lr_glb=1e-2):
        ...
        self.num_glb_epochs = int(getattr(args, "fedtgp_num_glb_epochs", num_glb_epochs))
        self.lr_glb = float(getattr(args, "fedtgp_lr_glb", lr_glb))
        num_prototypes = 1 if args.task == "graph_reg" else self.task.num_global_classes
        self.num_prototypes = num_prototypes
        self.trainable_prototypes = Trainable_prototypes(num_prototypes, args.hid_dim, args.hid_dim, device).to(device)
        self.gp_optimizer = torch.optim.Adam(self.trainable_prototypes.parameters(), lr=self.lr_glb, weight_decay=args.weight_decay)
    ...
    def execute(self):
        ...
        for client_i in self.message_pool["sampled_clients"]:
            for class_i in range(self.num_prototypes):
        ...
        row_id = [class_id for class_id in range(self.num_prototypes)]
            
        avg_proto = torch.zeros((self.num_prototypes, all_local_prototypes.shape[1])).to(self.device)
        ...
        for class_i in range(self.num_prototypes):
            count = max(y_list.count(class_i), 1)
            avg_proto[class_i] /= count
            
                
        gap = torch.ones(self.num_prototypes, device=self.device) * 1e9  
            

        for k1 in range(self.num_prototypes):
            for k2 in range(self.num_prototypes):
                if k1 > k2:
        ...
        for _ in range(self.num_glb_epochs):
            ...
            one_hot = F.one_hot(y, self.num_prototypes).to(self.device)
            gap_cap = float(getattr(self.args, "fedtgp_gap_cap", 100.0))
            gap_scale = float(getattr(self.args, "fedtgp_gap_scale", 1.0))
            gap2 = min(max_gap.item(), gap_cap) * gap_scale
            
            
            dist = dist + one_hot * gap2
            if self.args.task == "graph_reg":
                glb_loss = nn.MSELoss()(global_prototypes[0], avg_proto[0])
            else:
