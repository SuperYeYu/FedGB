...
from fedgb.algorithms.standard_fl.fedproto.fedproto_config import config
...
class FedProtoClient(BaseClient):
    ...
    def _prototype_mask(self, label, prototype_id):
        if self.args.task == "graph_reg":
            return self.task.train_mask
        return self.task.train_mask & (label == prototype_id)
        ...
        def custom_loss_fn(embedding, logits, label, mask):
            if self.message_pool["round"] == 0 or self.task.num_samples != label.shape[0]: # first round or eval on global
                ...
                for class_i in self._prototype_ids():
                    selected_idx = self._prototype_mask(label, class_i)
    ...
    def update_local_prototype(self):
        ...
        with torch.no_grad():
            ...
            for class_i in self._prototype_ids():
                selected_idx = self._prototype_mask(labels, class_i)
                if selected_idx.sum() == 0:
    ...
    def send_message(self):
        ...
        labels = self.task.data.y.to(self.device)
        for class_i in self._prototype_ids():
            prototype_counts[class_i] = int(self._prototype_mask(labels, class_i).sum().item())
