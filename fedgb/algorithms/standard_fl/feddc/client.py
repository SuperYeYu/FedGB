...
from fedgb.algorithms.standard_fl.feddc.feddc_config import config
    ...
    def get_custom_loss_fn(self):
        ...
        def custom_loss_fn(embedding, logits, label, mask):
            ...
            if self.message_pool["round"] != 0:
                ...
                drift_scale = float(getattr(self.args, "feddc_drift_scale", 1.0))
                grad_scale = float(getattr(self.args, "feddc_grad_scale", 1.0))
                return (
                    task_loss
                    + drift_scale * (config["feddc_alpha"] / 2) * loss_drift
                    + grad_scale * (1 / (self.args.lr * self.args.num_epochs)) * loss_grad
                )
            else:
                return task_loss
