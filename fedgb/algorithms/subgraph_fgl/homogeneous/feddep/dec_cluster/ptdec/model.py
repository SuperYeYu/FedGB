...
from fedgb.algorithms.subgraph_fgl.homogeneous.feddep.dec_cluster.ptdec.utils import target_distribution
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            if hasattr(dataset, "node_embs"):
                full_batch = dataset.node_embs.to(device)
                predicted = torch.argmax(model(full_batch), dim=1).detach().cpu()
            else:
                predicted = predict(
                    dataset,
                    model,
