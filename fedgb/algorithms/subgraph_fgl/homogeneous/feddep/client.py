...
from fedgb.algorithms.subgraph_fgl.homogeneous.feddep.localdep import LocalDGen, FedDEP, Classifier_F
from fedgb.algorithms.subgraph_fgl.homogeneous.feddep._utils import HideGraph, LocalRecLoss, FedRecLoss, GraphMender
...
class FedDEPClient(BaseClient):
    ...
    def __init__(self, args, client_id, data, data_dir, message_pool, device):
        ...
        cache_dir = os.path.join(data_dir, "feddep_cache")
        ...
        cache_path = os.path.join(cache_dir, f"client_{self.client_id}_{cache_key}.pt")
        cache_hit = os.path.exists(cache_path)
        if cache_hit:
            cached = torch.load(cache_path, weights_only=False)
            self.hide_data = cached["hide_data"]
            self.emb = cached["emb"]
            self.x_missing = cached["x_missing"]
            print(f"FedDEP client {self.client_id} hide graph cache hit.", flush=True)
        else:
            print(f"FedDEP client {self.client_id} hide graph start.")
            self.hide_data, self.emb, self.x_missing = self.hide_graph_model(data=self.data)
            print(f"FedDEP client {self.client_id} hide graph finish.")
            torch.save(
                {
                    "payload": cache_payload,
                    "hide_data": self.hide_data.detach().cpu(),
                    "emb": self.emb,
                    "x_missing": self.x_missing.detach().cpu(),
                },
                cache_path,
            )
            gc.collect()
        ...
        print(f"FedDEP client {self.client_id} init ready.", flush=True)
    ...
    def execute(self):
        ...
        if self.phase == 0:
            ...
            for i in range(config["feddep_epochs"]):
                print(f"Client {self.client_id} FedDEP dependency epoch {i} start...", flush=True)
                dep_grad = dict()
                ...
                for client_id in self.message_pool["sampled_clients"]:
                    if client_id != self.client_id:
                        ...
                        target_device = pred_embs.device
                        x_missing_t = torch.as_tensor(x_missing, dtype=pred_embs.dtype, device=target_device)
                        emb_t = torch.as_tensor(emb, dtype=pred_embs.dtype, device=target_device)
                        choice = torch.randint(
                            x_missing_t.shape[0], (embedding.shape[0],), device=target_device)
                        slot_choice = torch.randint(
                            x_missing_t.shape[1], (embedding.shape[0], config["num_preds"]), device=target_device)
                        sampled_missing = x_missing_t[choice]
                        row_idx = torch.arange(embedding.shape[0], device=target_device).view(-1, 1)
                        global_target_emb = sampled_missing[row_idx, slot_choice]
                        fallback_emb = emb_t[choice].unsqueeze(1).expand_as(global_target_emb)
                        global_target_emb = torch.where(
                            global_target_emb.abs().sum(dim=-1, keepdim=True) < 1e-15,
                            fallback_emb,
                            global_target_emb,
                        )
                ...
                for k, v in self.feddep_model.named_parameters():
                    v.grad += dep_grad[k]
                feddep_optim.step()
                print(f"Client {self.client_id} FedDEP dependency epoch {i} finish.", flush=True)
        else:
