...
from fedgb.algorithms.subgraph_fgl.homogeneous.fedsage_plus.fedsage_plus_config import config
import numpy as np
from fedgb.algorithms.subgraph_fgl.homogeneous.fedsage_plus.locsage_plus import LocSAGEPlus
from fedgb.algorithms.subgraph_fgl.homogeneous.fedsage_plus._utils import greedy_loss
from fedgb.data.simulation import get_subgraph_pyg_data
...
class FedSagePlusClient(BaseClient):
            ...
            def custom_loss_fn(embedding, logits, label, mask):    
                ...
                for client_id in self.message_pool["sampled_clients"]:
                    if client_id != self.client_id:
                        ...
                        global_target_feat = []
                        other_feat = self.message_pool[f"client_{client_id}"]["feat"]
                        for node_id in others_central_ids:
                            ...
                            for neig_id in others_neig_ids:
                                global_target_feat.append(other_feat[neig_id])
                        global_target_feat = torch.stack(global_target_feat, 0).view(-1, config["max_pred"], self.task.num_feats).to(pred_neig_feat.device)
    ...
    def execute(self):
        ...
        if self.phase == 0:
            ...
            self.task.model.output_pred_degree = None
            self.task.model.output_pred_neig_feat = None
            self.task.model.output_mend_graph = None
            gc.collect()
        else:
            with torch.no_grad():
    ...
    def send_message(self):
        ...
        self.message_pool[f"client_{self.client_id}"] = {
                ...
                "weight": [param.detach().cpu().clone() for param in self.task.model.parameters()],
            }

        if "round" not in self.message_pool or (hasattr(self, "phase") and self.phase == 0):
            self.message_pool[f"client_{self.client_id}"]["feat"] = self.task.data.x.detach().cpu()  # for 'loss_other'
