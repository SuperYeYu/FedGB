    ...
    def mend(self, impaired_data, pred_degree_float, pred_neig_feat):
        ...
        for impaired_node_i in range(num_impaired_nodes):
            for gen_neighbor_j in range(min(self.max_pred, pred_degree[impaired_node_i])):
                remain_feat.append(pred_neig_feat[impaired_node_i][gen_neighbor_j].reshape(1, -1))
                remain_edges.append(torch.tensor([impaired_node_i, ptr]).reshape(2, 1).to(impaired_data.edge_index.device))
                ptr += 1
        ...
        if pred_degree.sum() > 0:
            ...
            mend_y = torch.hstack((impaired_data.y, torch.zeros(ptr - num_impaired_nodes).to(impaired_data.y.device)))
        else:
