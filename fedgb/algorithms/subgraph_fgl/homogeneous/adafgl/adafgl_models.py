...
from fedgb.algorithms.subgraph_fgl.homogeneous.adafgl.adafgl_config import config
from fedgb.algorithms.subgraph_fgl.homogeneous.adafgl.label_propagation_models import NonParaLP
from fedgb.algorithms.subgraph_fgl.homogeneous.adafgl.op import LaplacianGraphOp, ConcatMessageOp
...
class AdaFGLModel(nn.Module):
    ...
    def preprocess(self, adj):
        ...
        num_nodes = self.soft_label.shape[0]
        if num_nodes > config["dense_relation_max_nodes"]:
            edge_u, edge_v = get_sparse_relation_edges(
                self.soft_label,
                topk=config["sparse_relation_topk"],
                block_size=config["sparse_relation_block_size"],
                threshold=config["sparse_relation_threshold"],
            )
            self.universal_re = None
        else:
        ...
        universal_re_smooth_adj = sp.coo_matrix((np.ones(len(edge_u), dtype=np.float32), (edge_u, edge_v)), shape=(num_nodes, num_nodes))
        ...
        if isinstance(row, Tensor) or isinstance(col, Tensor):
            ...
            self.adj = csr_matrix((edge_weight, (row, col)), shape=(num_nodes, num_nodes))
...
class HetePropagateLayer(nn.Module):
    ...
    def forward(self, feature, device, learnable_re=None):
        ...
        if learnable_re is None:
            return low_rank_relation_propagation(feature_emb)
...
class HetePropagateModel(nn.Module):
    ...
    def forward(self, ori_feature, smoothed_feature, processed_feature, universal_re, device):
        ...
        learnable_re = universal_re.to(device) if universal_re is not None else None
...
def get_sparse_relation_edges(emb, topk=16, block_size=2048, threshold=0.999):
    emb_softmax = nn.Softmax(dim=1)(emb.detach()).cpu()
    emb_softmax = emb_softmax / torch.norm(emb_softmax, p=2, dim=1, keepdim=True).clamp_min(1e-9)
    num_nodes = emb_softmax.shape[0]
    topk = max(1, min(int(topk), max(1, num_nodes - 1)))
    rows = []
    cols = []
    for start in range(0, num_nodes, block_size):
        end = min(start + block_size, num_nodes)
        sim = torch.mm(emb_softmax[start:end], emb_softmax.t())
        row_ids = torch.arange(start, end)
        sim[torch.arange(end - start), row_ids] = -1
        vals, idx = torch.topk(sim, k=topk, dim=1)
        keep = vals >= threshold
        empty_rows = torch.where(~keep.any(dim=1))[0]
        if empty_rows.numel() > 0:
            keep[empty_rows, 0] = True
        block_rows = row_ids[:, None].expand_as(idx)[keep]
        block_cols = idx[keep]
        rows.append(block_rows.numpy())
        cols.append(block_cols.numpy())
    return np.concatenate(rows), np.concatenate(cols)

def low_rank_relation_propagation(feature_emb):
    emb_softmax = F.softmax(feature_emb, dim=1)
    relation_basis = emb_softmax / torch.norm(emb_softmax, p=2, dim=1, keepdim=True).clamp_min(1e-9)
    relation_message = torch.mm(relation_basis, torch.mm(relation_basis.t(), feature_emb))
    return (F.softmax(relation_message, dim=1) + feature_emb) / 2
