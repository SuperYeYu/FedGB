...
class ProcessedSubgraphDataset:
    """Minimal dataset wrapper for FedGB preprocessed global subgraphs."""
    ...
    @staticmethod
    def _infer_num_classes(data):
        if hasattr(data, "node_types"):
            for node_type in data.node_types:
                labels = getattr(data[node_type], "y", None)
...
def load_processed_subgraph_dataset(path):
    ...
    if isinstance(payload, tuple):
        ...
        if isinstance(raw_data, dict) and data_cls is not None:
            data = data_cls.from_dict(raw_data)
        else:
            data = raw_data
    else:
        data = payload
    return ProcessedSubgraphDataset(data)
...
def load_global_dataset(root, scenario, dataset):
    ...
    if scenario == "graph_fl":
        processed_path = osp.join(root, "graph_fl", dataset, "processed", "data.pt")
        if osp.isfile(processed_path):
            ...
            if isinstance(payload, FGLGraphDataset):
                return payload
        if dataset in {"NOMAD_FGL", "PUBCHEM_FGL"}:
            return torch.load(
                osp.join(root, "graph_fl", dataset, "processed", "data.pt"),
                weights_only=False,
            )
        if dataset in  ["AIDS",
                        "BZR",
        ...
        processed_path = osp.join(root, "subgraph_fl", dataset.lower(), "processed", "data.pt")
        if osp.isfile(processed_path):
            return load_processed_subgraph_dataset(processed_path)
