...
from fedgb.utils.basic_utils import total_size
...
class Logger:
    ...
    def add_log(self, evaluation_result):
        ...
        if "cluster_indices" in server_message:
            cluster_indices = copy.deepcopy(server_message["cluster_indices"])
            evaluation_result["cluster_indices"] = cluster_indices
            evaluation_result["cluster_sizes"] = [len(cluster) for cluster in cluster_indices]
        self.metrics_list.append(evaluation_result)
        
        # cost
        if self.args.comm_cost:
            comm_cost = 0
            for client_id in self.message_pool["sampled_clients"]:
                client_message = self.message_pool.get(f'client_{client_id}')
                if client_message is not None:
                    comm_cost += total_size(client_message)
    ...
    def save(self):
        ...
        if self.args.comm_cost:
            log["comm_cost_per_round"] = self.comm_cost
            log["comm_cost_per_round_kb"] = [cost / 1024 for cost in self.comm_cost]
