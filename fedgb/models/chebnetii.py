...
from fedgb.models.pregc_common import ChebIIPropagation, scalar_edge_weight


class ChebNetIIEncoder(nn.Module):
    def __init__(self, input_dim, hid_dim, num_layers=2, dropout=0.5, k=10, dprate=0.0):
        super(ChebNetIIEncoder, self).__init__()
        ...
        for layer_id in range(max(1, num_layers)):
            ...
            self.propagations.append(ChebIIPropagation(k=k))
...
class ChebNetII(nn.Module):
    def __init__(self, input_dim, hid_dim, output_dim, num_layers=2, dropout=0.5, k=10, dprate=0.0):
        ...
        self.encoder = ChebNetIIEncoder(input_dim, hid_dim, num_layers, dropout, k, dprate)
